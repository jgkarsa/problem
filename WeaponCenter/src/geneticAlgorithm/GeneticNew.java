...
public class GeneticNew implements WeaponAlgorithm {
	...
	public GeneticNew(double[][] atkPro, int[] tarVal, Attribute AT) {
	...
	public void selection() {
		...
		for (ChromOneD chrom : ChromList) {
			...
			sum = sum + chrom.getBestValue();

		}
//		System.out.println("fron: " + sum);
//		System.out.println("size: " + ChromList.size());
		// use roulette wheel selection
		for (int i = 0; i < populationNum; i++) {
			double rand = Math.random();
			for (int j = 0; j < ChromList.size(); j++) {
				rand = rand - ((1 / ChromList.get(j).getBestValue()) / totalValue);
				if (rand < 0) {
